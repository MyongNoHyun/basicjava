...
public class OOPBasic {
	public static void main(String[] args) {
		...
		2. 객체지향언어
		 	- 기존의 프로그래밍 언어에 몇 가지 새로운 규칙을 추가한 보다 발전되 형태이다.
		...
		3. 클래스와 객체
		 ...
		 2) 클래스의 선언
		 	: [접근제어자][지정예약어] class 클래스명 [extends 클래스명][implements인터페이스]{}
		...
		4. 인스턴스란?
		  - 클래스로부터 객체를 만드느 과정을 인스턴스화라고 한다
		  - Tv클래스로부터 만들어진 객체를 Tv클래스의 인스턴스 라고 한다.
		   
		  - 인스턴스 생성방법
		  	: 클래스명 변수명; => 클래스이 객체를 참조하기 위한 참조변수를 선언한다.
		   	: 변수명 = new 클래스명(); => 클래스의 객체를 생성후, 객체의 주소를 참조변수에 저장한다.
		   	: 인스턴스는 참조변수를 통새서만 다를수 있다. 참조변수의 타입은 인스턴스의 타입과 일치해야 한다.
		...
		5. 선언위치에 따른 변수의 종류
		  ...
		  - 인스턴스변수
		 	  : 클래스의 인스턴스를 생성할깨 생성하때 만들어진다/
		 	  : 인스턴스 변수의 값을 읽어 오거나 저장하기 위해서는 먼저 인스턴스화를 해야 한다.
		 	  : 인스턴스는 독립적인 저장공간을 가지므로 서로다른 값을 가질수 있다.
		 	  : 인스턴스마다 고유한 상태를 유지해야 하는 경우 인스턴스 변수를 선언된다.
		 
		  - 지역변수
		 	  : 매서드내에 선언되며 매서드 내에서만 사용가능하다.
		 	  : 매서드 종료시 소멸된다.
		...
		6. 매서드 
		    ...
		    - 반복적으로 사용되는ㄴ 코들ㄹ 줄이기 위해서 사용한다.
		    	=>유지보수가 쉬워진다.
		    - 작성방법
		    
		    	반환타입 메서드명(변수타입 변수명){
		    		
		    		return;//있어도 되고 없어도 된다
		    	}
		    	ex)메서드명//호출(변수타입 변수명,변수타입 변수명.....//한개가 아니라 여러개일수도있다){//매서드의 선언
		    																	//{} 구현부	
		    	}
		...
		7. return
		  ...
		  - 반환값
		 		...
		 		: 있는 경우에는
		 		=> return 반환값;
		 		=> 반환타입과 반환값으 타입이 같아야한다.
		 		
		8. 클래스멤버와 인스턴스 멤버간의 참조와 호출
		 	- 같은 클래스의 멤버간에는 객체생성이나 참조변수 없이 참조할 수 없다.
		 	- 클래스 맴버들은 인스턴스 멤버들을 참조할 수 없다	
		 	- 인스턴스 멤버가 존재하는 시점에는 클래스 멤버가 존재한다.
		 	- 클래스멤버가 존재하는 시점에는 인스턴스멤보가 존재 할수도 있고 안할수도있다	
		
		9. 메서드 오버로딩
		 	- 한 클래스 내에 같은 이름의 매서드를 여러개 정의하는 것이다.
		 	- 대표적인 예가 println()이다
			- 조건
				: 매서드명이 같아야 한다.
				...
				: 리턴타입은 관련이 없다.
			- 장점
				: 매서드의 이름으로 구분되면, 한 클래스의 모든 매서드의 명은 달라야 한다.
				: 근본적으로 같은 기능을 하는 것인데 다른이름 표현되면 기억하기가 어렵다.
				
		10. 매개변수
			- 메서들르 호출할때 매개변수로 지정한 값(인자값)을 매서드의 매개변수에 복사하여 넘겨준다.
		 	- 기본형 매개변수
		 		: 단순히 저장된 값을 복사한다.
		 		: 변수의 값을 읽기만 할 수 있다.
		 	- 참조형 매개변수
		...
		12. JVM메모리 구조
			...
			- Call Stack
				...
				: 매서드가 호출되면 호출스택에 호출된 메서드를 위한 메모리가 할당되며
		...
		14. 생성자
			- 인스턴스가 생성 될때 호출되는 인스턴스 초기화 메서드 이다.
			- 특징
			  : 매서드처럼 클래스 내에 선언된다.
			  : 리턴값이 없다.
			   (생성자도 매서드 이기 때문에 리턴되는 값이 없으면 void를 변환타입에 적어줘야 하지만
			    모든 생성자는 리턴 값이 업식 때문에 void를 생략한다.)
		...
		15. 변수의 초기화
			- 변수를 선어하고 처음으로 값을 저장하는 것을 '변수의 초기화'라고 한다.
			- 명시적 초기화
				: 변수의 선언과 동시에 초기화를 한다.
				int a = 10; 
			- 생성자를 이용한 초기화
