...
public class InherItance {
	public static void main(String[] args) {
		/*
		1. 상속(is a : ~은 ~이다.)					//낮은것을 큰것을 비교
			- 기존의 클래스를 재사용하여 새로운 클래스를 작성한다
			- 적은 양의 코드로 새로운 클래스를 작성할수 있다
			- 새로 작성하고자 하는 클래스의 뒤에 상속하고자 하는 클래스의 이름과
			...
			- 맴버만 상속 받는다
			- 자손클래스의 멤버의 개수는 조상클래스보다 같거나 같다
			
		2. 포함(has a : ~은 ~를 포함하고 있다)
			- 상속이외에 클래스를 재사용하는 방법
		...
		3. 단일상속
		  ...
		  - VCR의 멤버명을 바꾼다면 그동안 VCR의 멤버를 사용하던 부분들을 모두 수정해 주어야 한다.
		  - 클래스간의 관걔가 보다 명확해지고 코드를 더욱 신뢰할 수 있다.
				
		4. Object클래스 (모든 클래스의 조상)		
		  - 모든 클래스의 상속 계층도의 가장위에 위치하는 조상 클래스이다.		
		  - 다른클래스로부터 상속받지 ㅇ낳는 클래스들은 자동적으로 Object클래스를 상속받는다.
		  
		  class Tv{
		  
		  }
		  =>컴파일후
		  class Tv extneds Object{
				
		  }		
				
		  - toSrting()나 equals()와 같은 매서드를 따로 정의하지않고 사용할수 있는것은
		  	Object클래스에 정의된 것들이기 때문이다.
		
		5. 오버라이드(Override)
		  - 조상클래스로부터 상속받은 메서드의 내용을 변경(재정의)하는 것.
		  - 오버라이드 되기 위한 조건 => 선언부가 일치 해야 한다.
			: 자손클래스에서 오버라이드 하는 매서드느 조상클래스의 매서드의 명과 동일해야한다.
			: 매개변수의 타입과 개수가 같아야 한다. !(타입x || 계수x) => 타입 o && 개수o
			: 리턴타입이 같아야 한다.
			: 접근제어자를 조상 클래스보다 좁은 범위로 설정할수 없다.		//이하 나중에 자세히 배움
			: 예외는 조상 클래스의 매서드보다 많이 선언할 수 없다.
		
		6. super.
		  - 조상클래스의 멤버와 자손클래스의 멤버가 중복정의되어 있을떄 서로 구분하기 위해 사용.
		  - 조상멤버와 자신의 멤버를 구별하는데 사용한다는 것을 제외하고는 this.과 근본적으로 동일
		  - static매서드에서 사용할 수 없다.
		...
		8. 제어자
		  ...
		  - static(클래스의, 공통적인)
		  	...
		  	: static메서드
		  	  -> 인스턴스 생성없이 사용 가능
		  	  -> 클래스 매서드 내에서 인스턴스 멤버를 호출할 수 없다.
		  
		  - final(마지막의, 변경할수 없는)
		    : 변수에 사용시 변경할수 없는 상수가 된다.
		    : 메서드에 사용 시 오버라이드 할수 없다.
		    : 클래스에 사용 시 상속할수 없는 클래스가 된다 
		
		  -  abstract( 추상의, 미완성의)
		   	: 매서드와 클래스에만 사용가능
		   	: 매서드에 사용시 구현부는 작성하지 않는다.
		   		void method();
		...
		9. 접근제어자
		  ...
		  - public : 접근제한이 없다
		  - 클래스에는 default, public사용 가능
		  
		10. 캡슐화
		  - 접근제어자를 사용하는 이유
		  	: 외부로 부터 데이터 를 보호하기 위한 목적
		...
		14. 매개변수의 다양성
		  - 참조변수의 다형적인 특지은 매서드의 매개변수에도 적용된다.
		  
		15. 여러객체 배열 , Vector
		  - 조상타입의 참조변수로 자손타입의 인스턴스를 참조할수있다
		  - Vector
		  	: 배열을 사용할 경우 전체 크기를 한번 정하면 변경할 수 있다.
		  	   이름 Vector클래스를 이용하여 해결할 수 있다.
		  	: 동적으로 크기가 관리되는 객체배열 이라고 할 수 있다.
		  	
			Vector v = new Vector(); //기본이 Object
			Vector() : 10개의 객체를 저장할수 있는 Vector인스턴스를 생성한다.
			add()	 : Vector에 객체를 추가한다.
			remove() : Vector에 객체를 삭제한다.
			isEmpty(): Vector에 객체를 가지고 있는지 확인 비어있으면 ture
			Object get(int index) : index번쨰 객체를 반환한다.
			int size() : Vector에 객체에 저장된 개수를 반환한다.
		
		16. 추상클래스(abstract class)
		  - 미완성된 클래스 : 미완성된 매서드를 가지고 있다는 의미
		  - 구성
		  	abstract class 클래스명{ 
		  ...
		  - 추상 메서드 
		  	...
		  	: 추상메서드는 선언부만 작성 되어 있고 구현부는 작성되지 않은 메서드를 의미한다.
		  	abstract 리턴타입 메서드명();// 중가로를 하면 안된다
		  - 추상클래스로부터 상속받는 자손 클래스는 오버라이딩을 통해 조상인 클래스의 
		  	추상메서드를 구현해 주면된다.
		
		17. 인터페이스 (interface)
		  - 일종의 추상클래스 이지만 추상메서드와 상수만 멤버로 가진다.
		  - 인터페이스의 작성
		  	interface 인터페이스명{ //스스로는 인스턴스화를 할수 없다
		  		public static final 변수타입 변수명 = 값;
		  		변수타입 변수명 = 값;
		  		public abstract 반환탕비 매서드명(매개변수타입 명,....);
		  		반환탕비 매서드명(매개변수타입 명,....);
		  	}
			
			: 모든 멤버변수는 public static final 이어야 한다. => 생략할 수 있다.
			: 모든 메서드는 public abstract이어야 한다. => 생략할 수 있다.
		  ...
		  - 인터페이스의 구현
			...
			: ~able 
				=> Movable - 예전
				=> IMove(인터페이스) => IMoveImpl (인프리먼츠) //회사마다 다르다.
		  - 인터페이스의 다형성
		  ...
		  - 인처페이스의 장점
		  	: 개발시간을 단축 할 수 있다.
			: 표준화가 가능하다.
			: 서로 관게가 없는 클래스들에게 관계를 맺어줄수 있다.
			: 독립적인 프로그래밍이 가능하다.
			: 7-26예제 꼭 해보세요~ //7장에 대해 거의다 담고있다.
			
		  - 인터페이스의 이해
		  	: 클래스를 사용한 쪽(user)과 클래스를 제공하는 쪽(provider)이 있다.//인터페이스 메서드 형태만 볼수 있다.형태를 볼수있음	
			 A(제공) - I - B(사용(인터페이스 선언부만 볼수있음))//JDk1.7   10년뒤에 메서드 지향(자바스크립트)
			 
		18. 내부클래스(inner class)
		  - 내부클래스란?
		  	: 클래스 내에 선언된 클래스
			: 서로 쉽게 접근할수 있고(?) 외부에서 볼때 불필요한 클래스를 감춤으로
			 복잡성(?)을 줄일 수 있다.
